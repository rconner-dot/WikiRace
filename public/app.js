...
(() => {
  ...
  async function showResult() {
    ...
    try {
      ...
      if (same.length) {
        ...
        tiles.push([`${best.clicks} · ${fmtPrecise(best.timeMs)}`, 'previous best']);
      }
    } catch (e) { /* ignore */ }
  ...
  async function onRoomState(room) {
    ...
    if (racing && myGame && (!state.game || state.game.id !== myGame) && state.pendingRoomGame !== myGame) {
      state.pendingRoomGame = myGame;
      try { await startRoomGame(room); } finally { state.pendingRoomGame = null; }
    }
